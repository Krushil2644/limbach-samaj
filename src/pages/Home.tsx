...
export default function Home() {
  const upcomingEvents = eventsData.filter(event => event.upcoming).slice(0, 3);

  return (
    <>
      <SEOHead
        title="Home"
        description="Limbach Samaaj represents Limbach families and community members across Canada. Join us for cultural events, community support, and meaningful connections."
        path="/"
      />
          ...
          <div className="container-custom">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
              {homeContent.features.map((feature, idx) => {
                const Icon = feature.icon === "Users" ? Users : feature.icon === "Calendar" ? Calendar : Heart;
                const colorClass = idx === 0 ? "bg-primary/10 text-primary" : idx === 1 ? "bg-secondary/10 text-secondary" : "bg-accent/10 text-accent";
                
                return (
                  <div key={idx} className="text-center p-6 hover-lift">
                    <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full ${colorClass} mb-4 transition-transform`}>
            ...
            {false && upcomingEvents.length > 0 && (
              <div className="text-center mt-8">
                <Button asChild variant="outline" size="lg" className="hover-lift">
                  <Link to="/events">
                    {homeContent.upcomingEvents.viewAllText} <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>
              </div>
            )}
          ...
          <div className="container-custom">
            <div className="bg-gradient-primary text-white rounded-2xl p-12 md:p-16 text-center hover-lift">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Button asChild size="lg" variant="outline" className="bg-white/10 backdrop-blur-sm text-white border-white hover:bg-white/20 hover-lift">
